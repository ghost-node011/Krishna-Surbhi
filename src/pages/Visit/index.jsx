...
const PRIMARY_BTN = 'inline-flex items-center justify-center gap-2 bg-forest text-white font-semibold text-[11px] tracking-wider uppercase px-8 py-4 rounded-full hover:bg-forest-dark transition-colors';
const OUTLINE_BTN = 'inline-flex items-center justify-center gap-2 border border-forest/25 text-forest font-semibold text-[11px] tracking-wider uppercase px-8 py-4 rounded-full hover:bg-forest hover:text-white transition-all';
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-8 mt-14 pt-10 border-t border-forest/10">
              {facts.map(({ Icon, title, text }) => (
                <div key={text} className="flex flex-col items-center gap-2">
                  ...
                  <div className="text-forest-dark text-sm font-semibold">{title}</div>
                  <div className="text-brown/55 text-xs">{text}</div>
                </div>
              ))}
            </div>
          </FadeIn>
        ...
        <div className="max-w-3xl mx-auto">
          <FadeIn>
              ...
              {VISIT.seasons.map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`block text-xs mt-0.5 ${active ? 'text-white/70' : 'text-brown/45'}`}>{s.months}</span>
                  </button>
                );
              })}
            </div>
            ...
            <div className="bg-white rounded-2xl border border-forest/10 p-5 md:px-6 flex flex-wrap items-center justify-between gap-3 mb-4">
              ...
              <div className="text-brown/60 text-sm">Last entry {season.lastEntry} · Closed {VISIT.closedDayLabel}</div>
            </div>
              ...
              {[
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-forest-dark font-semibold">{label}</div>
                    <div className="text-brown/50 text-xs mb-3">{days}</div>
                    <div className="font-serif text-4xl text-forest-dark">{inr(season[key])}</div>
                    <div className="text-brown/50 text-xs mt-1">per person</div>
                  </div>
                );
              })}
            </div>
            ...
            <div className="bg-forest-dark text-white rounded-2xl p-5 md:px-6 flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <div className="text-white/65 text-sm">Little ones under {VISIT.freeUnderAge} years</div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-forest/10 p-5 md:px-6 flex items-start gap-4">
              ...
              <div className="flex-grow">
                <div className="flex items-baseline justify-between gap-3 flex-wrap">
                  <div className="font-semibold text-forest-dark">{VISIT.basket.name}</div>
                  <div className="font-semibold text-forest-dark">
                    {inr(VISIT.basket.price)} <span className="text-brown/50 text-xs font-normal">each</span>
                  </div>
                </div>
                <p className="text-brown/60 text-sm mt-1 leading-relaxed">{VISIT.basket.contents}</p>
              </div>
            </div>
            ...
            <ol className="space-y-6">
              {VISIT.steps.map((s, i) => (
                <li key={s.title} className="flex gap-4">
                  ...
                  <div>
                    <div className="font-semibold text-forest-dark">{s.title}</div>
                    <p className="text-brown/60 text-sm leading-relaxed mt-0.5">{s.text}</p>
                  </div>
                </li>
              ))}
            </ol>
            <div className="mt-10 rounded-2xl bg-saffron/10 border border-saffron/20 p-5 text-sm text-brown/75 leading-relaxed">
              <strong className="text-forest-dark">Please note:</strong> {VISIT.note}
            </div>
          </FadeIn>
            ...
            <ul className="space-y-5">
              {VISIT.occasions.filter((o) => o.desc).map((o) => (
                <li key={o.label} className="flex gap-3">
                  ...
                  <div>
                    <div className="text-white font-semibold text-sm">{o.label}</div>
                    <div className="text-white/55 text-sm">{o.desc}</div>
                  </div>
                </li>
              ))}
            </ul>
              ...
              <ul className="space-y-5 mb-7">
                <li className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <a href={CONTACT.mapUrl} target="_blank" rel="noreferrer" className="text-forest text-xs font-semibold hover:underline">
                      Open in Google Maps ↗
                    </a>
                  </div>
                </li>
